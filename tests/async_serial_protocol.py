...
class SerialProtocolHandler(asyncio.Protocol):
    def __init__(self, queue, communicator):
        ...
        Args:
            ...
            communicator: AsyncSerialCommunicator 클래스의 self 객체 (AsyncSerialCommunicator 클래스의 ensure_connection함수를 활용해 연결 끊길 경우 연결 재시도를 위함)
        Functions:
            connection_made: serial connection이 문제 없이 처리되면 호출
            data_received: 데이터를 수신받으면 그 즉시 호출
            connection_lost: serial connection이 끊기면 호출, 연결 재시도 함수 호출
    ...
    def connection_lost(self, exc):
        ...
        asyncio.create_task(self.communicator.ensure_connection())
...
class AsyncSerialCommunicator:
    ...
    def __init__(self, port, baudrate):
        ...
        - ensure_connection(): 시리얼 연결 확인 및 재연결 시도
        - connect(): 시리얼 연결을 수립
        ...
        - 시리얼 연결 변수 생성 방법 : 새로운 프로토콜 객체를 만들어 돌려주는 팩토리 함수를 활용해서 SerialProtocolHandler(self.queue, self) 인스턴스를 새로 만들어 전달함.
            * 시리얼 연결이 열릴 때마다, 각 연결마다 독립적인 SerialProtocolHandler 객체가 필요.
            * 이 객체 내부에는 바이트 버퍼, 상태 정보, transport 속성 등이 들어 있기 때문에, 하나만 만들어 여러 연결에 재사용하면 충돌이 생길 수 있음.
    ...
    async def connect(self):
        ...
        self.transport, self.protocol = await serial_asyncio.create_serial_connection(
            loop, lambda: SerialProtocolHandler(self.queue, self), self.port, baudrate=self.baudrate,
            # timeout=0.5, write_timeout=0.5, rtscts=False, dsrdtr=False
        )
    ...
    async def ensure_connection(self, retry_delay: float = 1.0):
        """
        transport가 없거나 닫혀 있으면 connect()를 호출.
        실패 시 retry_delay 초마다 재시도.
        """
        while True:
            try:
                # transport 없거나 닫힌 상태면 재연결
                if not self.transport or self.transport.is_closing():
                    await self.connect()
                return
            except (serial.SerialException, OSError) as e:
                print(f"[Serial] connect failed: {e!r}. retry in {retry_delay}s…")
